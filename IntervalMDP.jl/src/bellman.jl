...
function bellman_max_changes!(Vres, V, k, model; upper_bound = false, maximize = true, max_changes, observed_actions)
    workspace = construct_workspace(model)
    strategy_cache = construct_strategy_cache(model)

    return bellman_max_changes!(
        workspace,
        strategy_cache,
        Vres,
        V,
...
function bellman_max_changes!(
    ...
    return _bellman_helper_max_changes!(
        workspace,
        strategy_cache,
        Vres,
        V,
...
function _bellman_helper_max_changes!(
    workspace::Union{DenseWorkspace, SparseWorkspace},
    strategy_cache,
    Vres,
    V,
    ...
    # Precompute permutations / sorting if needed
    # Does nothing in sparse workspaces.
    # bellman_precomputation!(workspace, V, upper_bound)

    for jₛ in 1:(length(stateptr) - 1)
        state_bellman_max_changes!(
            workspace,
            strategy_cache,
            Vres,
            V,
...
function state_bellman_max_changes!(
    workspace::Union{DenseWorkspace, SparseWorkspace},
    strategy_cache,
    Vres,
    V,
    ...
    @inbounds begin
        ...
        actions = @view workspace.actions[1:(s₂ - s₁)]
        j_prev = observed_actions[jₛ] + 1 # Julia is 1-indexed

        opt_val = maximize ? -Inf : Inf
        gt = maximize ? (>) : (<)
        opt_index = -1
        ...
        if max_changes == 0
            ...
            if k == 0
                opt_val = 0.0
                # opt_val = state_action_bellman(workspace, reward(prop), prob, s₁ + (j_prev-1), upper_bound)
            else
                opt_val = state_action_bellman(workspace, Vres[:, k, max_changes+1], prob, s₁ + (j_prev-1), upper_bound)
            end
        else
            for (i, jₐ) in enumerate(s₁:(s₂ - 1))
                index = i
                ...
                if k == 0
                    val = 0.0
                    # val = state_action_bellman(workspace, reward(prop), prob, jₐ, upper_bound)
                else
                    if i == j_prev
                        # Take the best value over all possible remaining changes
                        val = state_action_bellman(workspace, Vres[:, k, max_changes+1], prob, jₐ, upper_bound)
                    else
                        val = state_action_bellman(workspace, Vres[:, k, max_changes], prob, jₐ, upper_bound)            
                    end
                end
        ...
        strategy_cache[jₛ, k+1, max_changes+1] = opt_index
        Vres[jₛ, k+1, max_changes+1] = opt_val
    end
end
