...
function initialize_max_actions!(value_function, prop::AbstractReward)
    r = reward(prop)
    # TODO: not sure if we want to set just max_actions=0 or all max_actions
    value_function.current[:, 1, :] .= reward(prop)
end 
...
# TODO: need to do same with reachability
function step_postprocess_value_function_max_changes!(value_function, T::Int, max_changes::Int, prop::AbstractReward)
    rmul!(value_function.current[:, T+1, max_changes+1], discount(prop))
    value_function.current[:, T+1, max_changes+1] .+= reward(prop)
end
...
initialize_max_actions!(value_function, spec::Specification) =
    initialize_max_actions!(value_function, system_property(spec))
...
step_postprocess_value_function_max_changes!(value_function, T::Int, max_changes::Int, spec::Specification) =
    step_postprocess_value_function_max_changes!(value_function, T, max_changes::Int, system_property(spec))
