...
function IntervalProbabilities(lower::MR, gap::MR) where {R, MR <: AbstractMatrix{R}}
    ...
    if !(max_lower_bound <= 1 || isapprox(max_lower_bound, 1; atol=1e-10))
    ...
    min_upper_bound = minimum(sum_upper)
    
    if !(min_upper_bound >= 1 || isapprox(min_upper_bound, 1; atol=1e-10))
        @assert min_upper_bound >= 1 "The joint upper bound transition probability per column (min is $min_upper_bound) should be greater than or equal to 1."
    end
