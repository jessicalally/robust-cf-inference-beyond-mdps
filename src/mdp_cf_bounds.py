...
class CFBoundCalculator:
    def __init__(self, mdp, observed_state, observed_action, observed_next_state, transition_matrix):
        self.mdp = mdp
    ...
    def parallel_calculate_all_bounds(self, n_jobs=32):
        n_states = self.transition_matrix.shape[0]
        n_actions = self.transition_matrix.shape[1]
        ...
        def compute_chunk(chunk, lower_bound, upper_bound):
            results = []
            for s, a, s_prime in chunk:
                lb = Decimal(lower_bound(s, a, s_prime))
                assert not math.isnan(lb)
                ...
                if (ub < lb):
                    assert(math.isclose(lb, ub, rel_tol=1e-6))
                    lb = ub
        ...
        chunked_results = Parallel(n_jobs=n_jobs, backend="loky")(
            delayed(compute_chunk)(chunk, self.lower_bound, self.upper_bound)
            for chunk in chunks
        )
...
class MultiStepCFBoundCalculator:
    def __init__(self, mdp, transition_matrix):
        self.mdp = mdp
        self.transition_matrix = transition_matrix
    ...
    def calculate_bounds(self, trajectory):
        ...
        for t in range(n_timesteps):
            ...
            bound_calculator = CFBoundCalculator(self.mdp, trajectory[t][0], trajectory[t][2], trajectory[t][1], self.transition_matrix)
...
class ParallelMultiStepCFBoundCalculator:
    ...
    def calculate_bounds(self, trajectory, n_jobs=32):
        ...
        for t in range(n_timesteps):
            # print(f"Calculating bounds at time t={t}")
            bound_calculator = CFBoundCalculator(self.mdp, trajectory[t][0], trajectory[t][2], trajectory[t][1], self.transition_matrix)
            interval_cf_transition_matrix[t] = bound_calculator.parallel_calculate_all_bounds(n_jobs=n_jobs)
