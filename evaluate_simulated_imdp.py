...
def main():
    ...
    if function_name == "train":
        ...
        for i, (max_transitions, delta) in enumerate(hyperparameters):
            ...
            all_icfmdp_rewards = np.array(all_icfmdp_rewards).reshape(200000, 10)
            all_simulated_rewards = np.array(all_simulated_rewards).reshape(200000, 10)
        ...
        for i, (max_transitions, delta) in enumerate(hyperparameters):
            if max_transitions in rewards:
                rewards[max_transitions].append(mean_rewards[i])

            else:
                rewards[max_transitions] = [mean_rewards[i]]

        for num_transitions in possible_transitions:
            y_vals = rewards[num_transitions]
            
            x_vals = possible_deltas

            plt.scatter(x_vals, y_vals, label=f"Num Transitions =  {num_transitions}")

        plt.xlabel("Number of Transitions")
        plt.ylabel("Average Reward")
        plt.title("Average Reward vs. δ")
        plt.legend(title="Num Transitions")
        plt.grid(True)
