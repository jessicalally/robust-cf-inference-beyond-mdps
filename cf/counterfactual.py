...
class BatchSampler(object):
    ...
    def on_policy_sample(self, policy=None, n_steps=10, n_samps=1, out='array', 
        ...
        for samp_idx in range(n_samps):#tqdm(range(n_samps), disable=not(use_tqdm), desc=tqdm_desc):
    ...
    def cf_trajectory(self, batch, cf_policy, n_cf_samps=1,
        ...
        for obs_samp_idx in range(n_obs_eps): # tqdm(range(n_obs_eps), disable=not(use_tqdm), desc=tqdm_desc):
            for cf_samp_idx in range(n_cf_samps):
                ...
                for time_idx in range(n_obs_steps):
                    ...
                    # Record result
                    result[obs_samp_idx, cf_samp_idx, time_idx] = tmp
